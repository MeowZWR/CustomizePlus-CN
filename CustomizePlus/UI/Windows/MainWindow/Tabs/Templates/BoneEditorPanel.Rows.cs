...
public partial class BoneEditorPanel
{
    ...
    private void RenderChildScalingRow(BoneEditRow bone, BoneTransform transform)
    {
        ...
        using (var disabled = Im.Disabled(!_isUnlocked))
        {
            ...
            {
                if (DrawIconButton($"ChildLink{codename}", FontAwesomeIcon.Link, "切换独立子骨骼缩放。"))
                {
                    atomicEdit = true;
            ...
            CtrlHelper.AddHoverText(
                $"将「{BoneData.GetBoneDisplayName(codename)}」的子骨骼缩放与父骨骼缩放关联");
        }
        ...
        Im.Table.NextColumn();
        CtrlHelper.StaticLabel($"{displayName} - 子骨骼", CtrlHelper.TextAlignment.Left, "应用于子骨骼的缩放");
