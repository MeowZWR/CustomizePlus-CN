...
public class BoneEditorPanel
{
    ...
    public void Draw()
    {
        ...
        using (var style = Im.Style.Push(ImStyleDouble.ButtonTextAlign, new Vector2(0, 0.5f)))
        {
            ...
            using (var table = Im.Table.Begin("BoneEditorMenu"u8, 2))
            {
                ...
                Im.Line.Same();
                if (DrawIconButton("UndoBone", FontAwesomeIcon.Undo, "撤消", _undoStack.Count == 0))
                {
                ...
                Im.Line.Same();
                if (DrawIconButton("RedoBone", FontAwesomeIcon.Redo, "重做", _redoStack.Count == 0))
                {
                ...
                tableSize))
            {
                ...
                table.SetupColumn("骨骼"u8, TableColumnFlags.NoReorder | TableColumnFlags.WidthFixed, 6 * CtrlHelper.IconButtonWidth);
                ...
                if (showAllColumn)
                    table.SetupColumn("全部"u8, TableColumnFlags.NoReorder | TableColumnFlags.WidthStretch);

                table.SetupColumn("名称"u8, TableColumnFlags.NoReorder | TableColumnFlags.WidthStretch);
    ...
    private void DrawEditorConfirmationPopup()
    {
        ...
        Im.TextWrapped("您在当前模板有未保存的修改，请选择需要的操作。"u8);
        Im.Line.Spacing();
        Im.Separator();
        Im.Line.Spacing();
        ...
        Im.Line.Same();
        if (Im.Button("另存为副本"u8, buttonSize))
        {
        ...
        Im.Line.Same();
        if (Im.Button("不保存"u8, buttonSize))
        {
        ...
        Im.Line.Same();
        if (Im.Button("继续编辑"u8, buttonSize))
        {
            Im.Popup.CloseCurrent();
        }
    ...
    private void CompleteBoneEditor(BoneData.BoneFamily boneFamily, BoneEditRow bone)
    {
        ...
        Im.Table.NextColumn();
        if ((BoneData.IsIVCSCompatibleBone(codename) || boneFamily == BoneData.BoneFamily.未知 || boneFamily == BoneData.BoneFamily.摇晃 || boneFamily == BoneData.BoneFamily.自定义) && !codename.StartsWith("j_f_"))
        {
        ...
        CtrlHelper.StaticLabel(!isFavorite ? displayName : $"{displayName} ({boneFamily})", CtrlHelper.TextAlignment.Left,
            BoneData.IsIVCSCompatibleBone(codename) ? $"(IVCS 兼容) {codename}" : codename);
